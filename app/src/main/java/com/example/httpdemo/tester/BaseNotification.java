...
abstract class BaseNotification {
    ...
    protected static final HashSet<String> POLICY_KEYS = new HashSet<>(Arrays.asList(
            "start_time", "expire_time", "max_send_num", "in_app"
    ));
    ...
    public void useInApp(boolean inApp) throws Exception {
        if (inApp) {
            JSONObject obj = new JSONObject();
            obj.put("in_app", "true");
            obj.put("scene", "3"); //应用在前台时，转为浮窗展示通知消息
            setPredefinedKeyValue("in_app", obj);
        }
    }
}
